...
http://localhost:13128/4a/4a-001.php?template=../../../../../etc/hosts%00.html

``` sh
# %00 は NUL バイトで、そこでファイル名が終端される！！
curl 'http://localhost:13128/4a/4a-001.php?template=../../../../../etc/hosts%00.html'
```

- アプリケーションの実装によっては、書き込みや削除ができる場合がある
  - これどんなケース？
- PHP などのスクリプトファイルに書き込みがあると、スクリプトを Web サーバー上で実行することにより、任意のスクリプトを実行できてしまう！！
  - OS コマンド呼び出しの脆弱性と同じで、
- 3.4a-001
  - なんで php のところはコメントアウトされてるんだっけ？
    - php のスクリプトのところ
    - curl で叩くとシンプルに表示されてはいた！
    - ブラウザがよしなにコメントアウトしてくれていたらしい！！
  - curl 'http://localhost:13128/4a/4a-001.php\?template\=../4a-001b.php%00'
- 意図しないファイル公開
...
# terminal に出力しても、コピーしたら消えちゃう。。。？
printf 'わー\00い' | clipcopy

$$ hexdump -C <(printf 'a\x00b\n')
...
- 典型パターン
  - Local Exploit
- find `-exec` オプションがえぐい
  - 指定したファイル名に対してコマンドの実行とかある
- 脆弱性の原因
  - シェル経由で OS コマンドを呼び出す際に、シェルのメタ文字がエスケープされてない場合
