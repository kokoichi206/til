...
## sec10
マルウェアの親友やその他の活動は、ファイルシステムの変更によって検知できることが多い。
暗号学的ハッシュ関数の機能や小さなコマンドラインのツールを用いることで、追加削除変更されたファイルを特定することができる

STEP

1. システムの各ファイルのパスを記録する
2. システムの各ファイルのSHA-1ハッシュを生成する
3. ツールを後で再実行することで、変更削除移動新規作成されたファイルを出力する

`sdiff`を利用

sdiffは２つのファイルを比較し、差異を出力する
