...
## sec 4
UI テストの自動化は、ユニットテストと比較してテストを書くコストやメンテナンスするコストが大きくなりがち。

### UI テストの自動化を始める前に
- 画面デザインが変更されるたびに、テストコードの修正が必要
- 画面の操作があるため個々のテストケースの実行時間が長くなりやすい
- UI に起因する理由により、テスト結果が安定しない
    - UI を操作してから画面が変化するまでの時間が一定ではない
...
UI テストの自動化では、自動化にかかる投資が検証コストの減少だけでは回収できないことがある！

### 長くテストコードを利用し続ける
- テストコードに PageObject デザインパターンを適応する
- CI 環境で動かし続ける
- テスト結果をわかりやすく共有する
- 不安定なテスト対策をする
- 実行時間を短縮させる
