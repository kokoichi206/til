...
- ソフトウェアアーキテクチャは、以下の2つの側面を反映する
  - 企業の組織構造
  - 人々がどのようにコミュニケーションを取るか
- 急がば回れ
  - 情報を鵜呑みにしてはならない
  - 厄介な質問を避けてはいけない
- **指標が目標になった時、それは良い指標ではなくなる**
  - グッドハートの法則
  - 「KPI X や指標 Y を達成できてない」という言葉を聞いたら注意！
- デプロイの頻度を上げるべき
  - コスト、恐怖、不確実性が減る
  - 標準化、予測可能性、自動化が進む
- ソフトウェアアーキテクチャの自然な進化は、**メトリクスによって導かれるべき**
  - **自分のコンテキストにあったメトリクスを使う！**
    - 他人のものが使えないことも多い
...
- ソフトウェアメトリクス
  - 厳格な品質測定の面において、ソフトウェア産業は他の産業に比べて明らかに遅れている
- **big ball of mud**
  - アーキテクチャ上のまとまりを失った、ひどくもつれたコードベース
- サイズと複雑さを計測するメトリクス
  - １ファイルあたりの **LOC (Lines Of Codes)**
    - LOC の定義では空行やコメント行を除く
    - 合計行といった場合には、コメントや空行も含めておく
  - **ステートメント数**
    - 関数やメソッドのサイズ
    - 100 ステートメントが目安
  - インデントによる負債
  ...
  - **変更履歴から得られるメトリクス**
- コンポーネントランク
  - Google のページランクというメトリクス
- LCOM4
  - Lack of Cohesion of Methods
  - クラスが単一責任の法則に違反してないかを把握するためのメトリクス
- アーキテクチャ適応度関数
  - イリティとも呼ばれる
  - アーキテクチャで達成したい目標
  - 安定性、スケーラビリティ、保守性、アジリティなど
    - これらのどれか1つまたは複数をどの程度満たしているかを計測するもの
      - e.g. 相対的循環度、保守性レベル
    - **トレードオフを見極める**
- 同時に満たすことができないものもある
  - 最大限のパフォーマンスと最大限のセキュリティは相反する
    - 暗号化には多くの CPU パワーが使われるから
- いくつかの黄金律
  - アーキテクチャモデルを組織として持つ
    - 結合を抑制、最小化する
  ...
  - コードの重複を避ける
  - ソースファイルのサイズを　800LOC に制限する（ソフトな閾値）
  - 最大インデントを 4, 修正循環複雑どを 15 に制限する（ソフトな閾値）

## sec 10

- GQM: ゴール・クエスチョン・メトリクス
  - データを使うメトリクス
- GQM の根底
  - **なぜそれを計測するか**を理解しなければならない
- アプローチ
  - GQM のモデルは階層的
    - **根本にあるのがゴール**
  - ゴールを定義したらそれを探求し、特徴づけるための**質問（クエスチョン）**を行える
    - ゴールに近づいているのか、それとも遠ざかっているのか
  - れい？
    - 現在の可用性はどの程度か
    - どのコンポーネントが最も良い可溶性を備えているか、悪いのはどれか
    - どのコンポーネント・サービスが最もダウンしているか
    - 利用できなくなるのはなぜか
    - 典型的な利用停止はどのくらいか
    - 障害はいつ発生するか
- ADR
...
- [Move Fast and Fix Things](https://github.blog/2015-12-15-move-fast/)
