...
- プロダクションコードの分類
  - 以下２軸で４象限に分類される
    - コードの複雑さ・ドメインにおける重要性
    - 協力者オブジェクトの数
  - ドメインモデル、取るにたらないコード、コントローラ、過度に複雑なコード
- **Humble Object**
  - **コードの深さとコードの広さ**に分けて、それが混ざらないようにする
- 変換ロジック
  - ライブラリ等で、使う側からの見えない分岐が多い
    - => 何かの間違いが起こる可能性が高い
- リファクタリングえぐ
- **事前条件はテストすべきか**
  - 確固とすべきルールはないが、ドメインにおいて重要であればすべき
- **ドメイン・イベント**
  - ドメインモデルの状態を追跡する
  - ドメイン・エキスパートにとって意味のあるアプリケーションのイベント
  - [Merging Domain Events](https://enterprisecraftsmanship.com/posts/merging-domain-events-dispatching/)
  - コントローラーから決定を下すことに関する責務を抽出し、その責務をドメイン・モデルに担わせられるようになる！
...
- 結合テストとは
  ...
  - 単体テストはドメインモデルを検証
  - 結合テストはドメインモデルとプロセス外依存を結びつけるコードを検証
- **過度に複雑なコードは、テスをと書く前に、ドメインモデル or コントローラに分解すべき！**
- use mock?
  - **管理下にある依存**: managed dependency
    - 外部から見ることができない
    - アプリ経由でしかアクセスされない db など
      - 対象のアプリが最終的にどのような状態になるのかを外部の視点から検証しやすくなる
      - db のものにもしやすくなる
  - **管理下にない依存**: unmanaged dependency
    - 外部から見ることができる
- 結合テストの基本的な指針
  - **最長のハッピーパス**
  - **単体テストでは検証できなかった全ての edge ケース**
- インターフェイスを使った依存の抽象化
  - **プロセス外依存に対して、実装が1つしかないのにインタフェースを導入？**
    - 実装が1つしかないなら、そのインタフェースは抽象ではない
    - そして疎結合にもならない
    - **本来抽象化とは発見するもので、作り出すものではない**
  - **モックを作れるようにするのが目的**
    - つまり、**管理下にない依存に対してのみインタフェースを用意するべき**
- ログ出力に対するテスト
  - **ログ出力が、アプリケーションの観測可能な振る舞いの一部かどうか、実装の詳細かどうか**
  - 外部から見られることを意図しているのであれば、観測可能な振る舞い
    - => 結合テストで検証すべき
  - 出力されたログを見るのが**開発者だけの場合**
    - => 実装の詳細であり、テストされるべきではない
- 単体テストではプロセス外依存を扱うべきではない
- 複数の実行フェーズ（Act）を持ったテストケースを許容できるのは、開発者が必要なプロセス外依存を好きなように扱えない時だけ
- 統合テストの場合も複数の Act が意味があることはほとんどない
- ログ
  - ログ自体が『観測可能な振る舞い』かどうか
  - ログの種類
    - サポート・ログ
      - サポート・スタッフによってみられることを意識したもの
    - 診断ログ
      - 開発者が内部の状態を把握するためのもの
- 結合テスト
  - 優れた退行に対する保護とリファクタリングの耐性

## モック

- 管理下にない依存のみを mock 化すべき
  - **アプリケーションの境界に位置するもの！！**
- スパイは手書きのモック
- mock を使うのは統合テストの時のみ
...
- 統合テストではここも扱いたい
- db の準備
  - そのデータが **参照データ** か通常データか
- **Database クラスをリポジトリとトランザクションのクラスに分ける**トランザクションのクラスに分ける**
- 2つの決定を分離
  - どのデータを変更するのか
  - 変更したデータを db に反映するのか、反映しないのか
- リポジトリ
  - db にアクセスしてそのデータを変更するクラス
  - リポジトリはデータベースの操作を抽象化して使いやすくしたもの
  - 統合テストで検証すべき？ => NO
- トランザクション
  - atomic な更新を可能にする
- UserRepository クラスがコンストラクタの引数で Transaction オブジェクトを受け取る
- UserController もトランザクションを受け取る
- **Unit Of Work パターン**
- テストケース実行前にデータの後始末を行う！

## アンチパターン

- プライベートなメソッドの単体テスト
  - 退行に対する保護を失うことになる
  - プライベートなメソッドは実装の詳細につながるものであり、実装の詳細がテストされるようになると、そのテストは壊れやすいものになるため
- プライベートな状態の公開
- テストへのドメイン知識の漏洩
  - Add のテストで expected = 1 + 3 で定義するようなもの
- プロダクションコードへの汚染
- 具象クラスに対するテスト・ダブル
