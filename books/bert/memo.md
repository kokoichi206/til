...
### マルチラベルのデータ表現
文章が表すカテゴリーを **Multi-hot ベクトル**と呼ばれる形式で表現する。Multi-hot ベクトルは、要素が０または１からなるベクトル。

### データセット：chABSA-dataset
TIS 株式会社が公開している、上場企業の有価証券報告書を用いて作成されたマルチラベルのネガポジ判定のデータセット「chABSA-dataset」
...
### 固有表現抽出とは
文章から人名・組織名といった固有名詞、日付などの時間表現、金額などの数値表現を抽出する技術

### 文字列の正規化
「ＡＢＣ株式会社」と「ABC株式会社」は通常であれば同じものを意味する。しかし、コンピュータ上では全角と半角が異なる記号として扱われるので、そこの表記ブレを直す作業を行う。

Python では、`unicodedata.normalize`が用意されている
...
### 固有表現抽出の実装：IO 法
簡易な方法であるIO法についてまず理解する

#### タグ付け
固有表現抽出を実装するときには、各トークンにタグを与えることにより、文章中のどの文字列が固有表現なのかということと、それぞれの固有表現のカテゴリーを同時に表現する。

トーンのタグ付けには、IO法が容易である。IO法では多くの場合に有効であるものの、一部対応できないケースもある。

- トークンが固有表現の一部であれば、そのトークンのタグは`I-(TYPE)`とする。TYPEは固有表現のタイプを表す文字列で、人名の場合には`I-(人名)`となる
    - Inside
- トークンが固有表現の一部でなければ、そのトークンのタグは o とする
    - Outside

#### BERT による固有表現抽出
固有表現抽出は、与えられた文章をトークン化し、それぞれのトークンのラベルを予測する分類問題として扱うことができる。Transformers では、トークン単位の分類を行うためのクラス、BertForTokenClassification が提供されている。

### dataset
ストックマーク社により公開されている、日本語版の Wikipedia から作成された固有表現抽出データセット

[データ詳細](https://github.com/stockmarkteam/ner-wikipedia-dataset)


### BIO法
IO法では、同じタイプの固有表現が連続する場合に、それらを連結したものが小湯表現として認識されてしまうという問題があった。そのため、一般的に、固有表現の**境界には**別のラベルを付与する方法が用いられる。よく使われるのは以下の**BIO法**
...
### 文章校正とは
一般的に、文章校正はさまざまなタイプの間違いの校正を対象とするが、本性では入力時における漢字の誤変換を校正するタスクを扱う。
...
### BERT による文章校正
BERTで実装するために、文章校正のタスクをトークン単位の分類問題として考える。

トークン化したときに誤変換を含む文章と正しい文章でトークン同士の対応関係がつかない例も存在し、BERTでこれらのパターンに対応すると難しくなるため、今回は扱わない

### トークナイザ
以下の３つの関数を用意する

- 誤変換した文章と正しい文章が与えられたときに、それぞれの文章を符号化し、モデルに入力できる形式にする関数 [推論]
- 誤変換した文章を符号化するとともに、各トークンの文章中での位置を特定する関数 [推論]
- BERTが出力したトークン列が与えられたときに、空白を考慮して、それを文章として出力する関数 [推論]
...
### 文章ベクトル
文章から文章ベクトルを生成するためにBERTを使う。BERTは文章のトークン列を入力すると、それぞれのトークンに対してベクトルを出力する。文章ベクトルを生成するためには、**各トークンに付与されたベクトルの組を集約して、単一のベクトルにする必要がある！**

BERTの出力を集約し文章ベクトルにする方法は２つ

- BERTから出力されたベクトルのうち、トークン列の先頭の特殊トークン[CLS]に対応するベクトルを用いる
    - BERT元論文
- BERTから出力されたベクトルを平均したものを用いる
    - こっちの方が性能高い、という報告例

### 文章コーパスの可視化
文章ベクトルは、一般には数百次元のベクトルとなり、その分布を人間が理解するのは簡単ではない。文章ベクトルがどのように分布しているのかを直感的に理解するための方法として、高次元のベクトルを低次元の空間にマッピングする**次元圧縮**がよく行われる。次元圧縮の方法としては、

- 主成分分析
    - データのばらつきが大きな方向を選び、その方向にデータを線形に射影する
- t-SNE
    - 高次元の空間上で距離の低い点同士は低次元の空間でも距離が近くなるように、高次元の空間上で遠い点同士は低次元の空間ではより遠くになるように配置する非線形の手法
    - 主成分分析と比べるとデータの局所的・大域的な構造をうまく捉えることができる

などがある。どちらも、`scikit-learn`というライブラリで利用可能
...
livedoor のデータ、カテゴリー毎の分布にオーバーラップが見られるが、高次元ではなかったものか検討がつかない。
...
### 類似文章検索
内容が似ている文章同士は文章ベクトルもにている。
そこで、BERT で文章ベクトルを計算し、文章ベクトル間の類似度を選ぶ！
